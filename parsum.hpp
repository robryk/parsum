...
template<typename T> class reducing_snapshot {
	...
	private:
		...
		int child_idx(int tid) {
			return tid % 2;
		}

		int child_tid(int tid) {
			return tid / 2;
		}
		...
		std::unique_ptr<holder<latest_value, RawHolder>[]> latest_; // node only
		std::unique_ptr<reducing_snapshot> children_[2]; // node only
		...
		void update_last(intptr_t tid) {
			...
			if (!ok)
				return;
			intptr_t version_after = get_version();
		...
		update_result leaf_update(const T* val) {
			RL_ASSERT(leaf_);
			update_result res;
			intptr_t old_version = get(&res.value_before);
			RL_ASSERT(old_version != INVALID_VERSION);
			value v;
			v.child_values[0] = *val;
			bool ok = history_.publish(0, old_version+1, &v);
			RL_ASSERT(ok);
			res.update_count = old_version + 1;
			res.version = old_version + 1;
			return res;
		}

		bool leaf_get_last(update_result* output) {
			RL_ASSERT(leaf_);
			intptr_t version = history_.get_version();
			if (version == 0) {
				output->update_count = 0;
				output->version = 0;
				output->value_before = T();
				return true;
			}
			value buffer;
			if (!history_.get(version-1, &buffer))
				return false;
			output->update_count = version;
			output->version = version;
			output->value_before = buffer.child_values[0];
			return true;
		}

	public:
		reducing_snapshot(int thread_count, string name = "") :
			name_(name),
			thread_count_(thread_count),
			leaf_(thread_count == 1),
			history_(3*thread_count, thread_count),
			latest_(new holder<latest_value, RawHolder>[thread_count])
		{
			if (!leaf_) {
				children_[0].reset(new reducing_snapshot((thread_count + 1) / 2, name_ + "0"));
				children_[1].reset(new reducing_snapshot(thread_count / 2, name_ + "1"));
			}
		}

		constexpr static int INVALID_VERSION = -1;

		intptr_t get(T* result) {
			intptr_t version = history_.get_version();
			value buffer;
			if (!history_.get(version, &buffer))
				return INVALID_VERSION;
			if (leaf_)
				*result = buffer.child_values[0];
			else
				*result = buffer.child_values[0] + buffer.child_values[1];
			return version;
		}

		intptr_t get_version() {
			return history_.get_version();
		}

		bool get_last(intptr_t tid, update_result* output) {
			if (leaf_)
				return leaf_get_last(output);
			update_last(tid);
		...
		update_result update(intptr_t tid, const T* val) {
			if (leaf_)
				return leaf_update(val);
			...
			do {
				intptr_t my_version = history_.get_version();
				value my_value;
